...
def w1m(
    ...
    score = jetnet.evaluation.gen_metrics.w1m(
        jets1=_pp_for_jetnet_metric(gen_batch)[:1000, ..., :3],
        jets2=_pp_for_jetnet_metric(sim_batch)[:1000, ..., :3],
    )[0]
...
def w1efp(
    ...
    score = jetnet.evaluation.gen_metrics.w1efp(
        jets1=_pp_for_jetnet_metric(gen_batch)[:1000, ..., :3].cpu(),
        jets2=_pp_for_jetnet_metric(sim_batch)[:1000, ..., :3].cpu(),
        num_batches=1,
        efp_jobs=10,
    )[0]
...
def fpnd(gen_batch: Batch, **kwargs) -> Union[float, torch.Tensor, np.float32]:
    try:
        score = jetnet.evaluation.gen_metrics.fpnd(
            jets=_pp_for_jetnet_metric(gen_batch)[:1000, ..., :3],
            jet_type="t",
            use_tqdm=False,
        )
        return float(score)
