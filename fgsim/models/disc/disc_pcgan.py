...
class ModelClass(nn.Module):
    def __init__(
        ...
        if modus == "latent":
            self.net = latent_D(
                z1_dim=conf.loader.n_points * conf.loader.n_features
            )
        elif modus == "regular":
            self.net = D(x_dim=conf.loader.batch_size, z1_dim=z1_dim, d_dim=d_dim)
        elif modus == "skip":
            self.net = skipD(
                x_dim=conf.loader.batch_size, z1_dim=z1_dim, d_dim=d_dim
            )
        else:
            raise Exception
    ...
    def forward(self, batch):
        return self.net(
            batch.x.reshape(-1, conf.loader.n_points * conf.loader.n_features)
        )
