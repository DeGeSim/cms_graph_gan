...
def validate(holder: Holder, loader: QueuedDataLoader, train_log: TrainLog) -> None:
    holder.model.eval()
    ...
    if not conf.debug:
        train_log.writer.add_scalar(
            "val_loss", mean_loss, holder.state["grad_step"]
        )
    ...
    if (
        ...
        if not conf.debug:
            train_log.experiment.log_metric("min_val_loss", mean_loss)
            train_log.experiment.log_metric(
